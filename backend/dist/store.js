...
exports.storeSamples = storeSamples;
...
async function storeSamples(samples) {
    if (!isDbConnected || samples.length === 0)
        return;
    const client = await pool.connect();
    try {
        const query = `
...
async function createSession(sessionId, userId = 'user1') {
    if (!isDbConnected)
        return;
    const client = await pool.connect();
    try {
        await client.query('INSERT INTO sessions (id, userId, startTime) VALUES ($1, $2, NOW()) ON CONFLICT (id) DO NOTHING', [sessionId, userId]);
    }
    finally {
        client.release();
    }
