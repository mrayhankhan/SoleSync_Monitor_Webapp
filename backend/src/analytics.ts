...
import type { SensorSample } from './parser';

// State for each device/foot
interface DeviceState {
    madgwick: any; // AHRS instance
    lastStepTime: number;
    stepCount: number;
    // Buffer for peak detection if needed
}

const deviceStates: Record<string, DeviceState> = {};

export interface ProcessedSample extends SensorSample {
    orientation: { yaw: number, pitch: number, roll: number, quaternion: { w: number, x: number, y: number, z: number } };
    isStep: boolean;
    stepCount: number;
    gaitPhase: string; // 'stance' | 'swing' etc.
}

function getDeviceState(deviceId: string): DeviceState {
    if (!deviceStates[deviceId]) {
        deviceStates[deviceId] = {
            madgwick: new Madgwick({ sampleInterval: 10, algorithm: 'Madgwick', beta: 0.1 }), // Assume ~100Hz for now, can be dynamic
            lastStepTime: 0,
            stepCount: 0
        };
    }
...
export function processSamples(samples: SensorSample[]): ProcessedSample[] {
