export interface SensorSample {
    ...
    fsr: [number, number, number, number, number];
    heelRaw: number;
    voltage?: number;
    temperature?: number;
    sessionId: string;
...
export function parseCsvChunk(csvChunk: string, sessionId: string): SensorSample[] {
    const lines = csvChunk.split('\n');
    const samples: SensorSample[] = [];

    for (const line of lines) {
        if (!line.trim() || line.startsWith('timestamp')) continue; // Skip empty or header

        const parts = line.split(',');
        if (parts.length < 15) continue; // Invalid row
        ...
        try {
            ...
            const foot = parts[2].toLowerCase() as 'left' | 'right';
